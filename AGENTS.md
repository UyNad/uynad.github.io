...
`Codex/` 目录存放可复用的任务文档。当用户提出对应任务时，先阅读对应文档再执行：

- **「刷新 PSN 数据」**：执行 `Codex/PSNBannerUpdate.md`（更新首页 PSN 横幅数据，需要 Chrome 扩展）
...
- 本地预览：`.Codex/launch.json` 已配置 hexo 服务（4001 端口）；本机 preview_screenshot 工具有故障，用 DOM 检查代替截图验证
